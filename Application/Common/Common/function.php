...
function getpage1($count, $pagesize = 10) {
    $p = new Think\Page($count, $pagesize);
    $p->setConfig('header', '');
    $p->setConfig('prev', '<button class="btn btn-white" type="button"><i class="fa fa-chevron-left"></i></button>');
    $p->setConfig('next', '<button class="btn btn-white" type="button"><i class="fa fa-chevron-right"></i></button>');
    $p->setConfig('last', '末页');
    $p->setConfig('first', '首页');
    $p->setConfig('theme', '%FIRST%%UP_PAGE%%LINK_PAGE%%DOWN_PAGE%%END%%HEADER%');
    $p->lastSuffix = false;//最后一页不显示为总页数
