...
class PointsmallController extends CommonController
{   //积分商城
    ...
    public function cateAddEditDelStatus()
    {   //添加修改删除状态
        if(IS_POST){
    ...
    public function goodsAddEdit()
    {
        ...
        $taglists = D("GoodsTag")->lists($where);
        $this->assign('taglists',$taglists);
        $this->display();
    }
    public function goodsAddEditDelStatus()
    {
        if(IS_POST){
