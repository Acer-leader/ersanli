...
class WxPayApi
{
	...
	/** 
	 * 
	 * APP支付 执行第二次签名，才能返回给客户端使用
	 *  参与签名的字段名为appId，partnerId，prepayId，nonceStr，timeStamp，package。
	 *  
	 */
	public static function getOrder($inputObj, $timeOut = 6)
	{
		if(!$inputObj->IsPrepay_idSet()){
			throw new WxPayException("第二次签名，缺少必填参数prepay_id！");
		}
	
		$inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
		$inputObj->SetPartnerid(WxPayConfig::MCHID);//商户号
		$inputObj->SetTimeStamp(time());//时间戳
		$inputObj->SetNonceStr(self::getNonceStr());//随机字符串
		$inputObj->SetPackage("Sign=WXPay");//扩展字段
